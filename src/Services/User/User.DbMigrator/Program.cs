...
// AddPersistence registers UserDbContext (Npgsql + snake_case naming) plus outbox/inbox,
// repositories and the Elasticsearch client - the same wiring User.API uses. The Elasticsearch
// client is a lazily-constructed singleton (see BuildingBlock.Search), so registering it here
// doesn't require Elasticsearch:Url to be configured for the migrator; it's never resolved.
...
try
{
    ...
    await using (var scope = provider.CreateAsyncScope())
    {
        ...
        // UserSeeder isn't DI-registered elsewhere (User.API never seeded); it only depends on
        // UserDbContext, so it's constructed directly rather than adding a registration just for this.
...
// database itself - on a first-time deployment the API crashes at startup because user_hangfire_db
