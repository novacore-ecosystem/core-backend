...
// AddPersistence registers ContentDbContext (Npgsql + snake_case naming) plus outbox/inbox and
// repositories - the same wiring Content.API used to do inline.
...
try
{
    ...
    await using (var scope = provider.CreateAsyncScope())
    {
        ...
        // ContentSeeder isn't DI-registered elsewhere (Content.API never seeded); it only depends
        // on ContentDbContext, so it's constructed directly rather than adding a registration
        // just for this.
...
// database itself - on a first-time deployment the API crashes at startup because content_hangfire_db
