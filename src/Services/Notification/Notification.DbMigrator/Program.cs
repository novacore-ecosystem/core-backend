...
try
{
    ...
    // Notification's own data lives in MongoDB (schemaless - no EF Core migrations). No seed
    // data exists anywhere in the codebase for it either - no seeder class, no default-channel/
    // template reference data - so nothing is fabricated here; seeding can be added later if a
    // real default catalog is introduced. Notification does consume Hangfire (Postgres-backed)
    // for background jobs, so that database still needs the same first-deploy provisioning every
...
// notification_hangfire_db doesn't exist yet. CREATE DATABASE can't run against the database
// being created, so this connects to the server's always-present 'postgres' maintenance database
// to check/create the real target.
