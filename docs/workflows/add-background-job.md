...
3. If the job needs config (thresholds, batch sizes), add an options class implementing `ISetting` (`BuildingBlock.Infrastructure.Configurations`) so it's auto-bound and fail-fast validated at startup, injected via `IOptions<T>` — see `RefreshTokenSyncService`/`AuthSchedulerSetting` (service-specific, FluentValidation-backed) or `OutboxCleanupJob`/`OutboxCleanupOptions` (shared) for the pattern.
