...
| [conventions/personal-coding-standards.md](conventions/personal-coding-standards.md) | **Reusable, project-agnostic** C# style rules — language/syntax currency, external data naming, warning-free code, XML doc rules, member ordering, regions, boolean naming, anti-hardcoding, static lookups, enum trailing commas. Portable to any C# backend project; explicitly separates itself from the NovaCore-specific rules in the doc above. |
| [conventions/coding-standards.vi.md](conventions/coding-standards.vi.md) | **Vietnamese, quick-reference only** — concise summary for developers onboarding onto NovaCore. Not a source of truth; always defers to the English docs above on any conflict. |
