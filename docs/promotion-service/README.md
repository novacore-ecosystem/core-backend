...
See [planning/PROGRESS.md](planning/PROGRESS.md). **Phase 0-3 complete, Phase 4 (Application/CQRS/API Skeleton) in progress — see planning/PROGRESS.md for how the "X / 7" marker works (current-phase position, not a completed count).** Persistence (Phase 3) is fully closed: all 103 entities have an `IEntityTypeConfiguration<T>`, `PromotionDbContext` is fully wired, the repository/Read/Write Persistence Service skeleton exists for all 11 true Aggregate Roots, and public Coupon search infrastructure is wired per [search/search-strategy.md](search/search-strategy.md). Phase 4.1 built one representative Coupon Command + Query CQRS/Minimal API flow (structural skeleton, handlers stubbed with `NotImplementedException`) per [cqrs/cqrs-strategy.md](cqrs/cqrs-strategy.md). The full solution builds clean through `Promotion.API`. No real Coupon feature, migration, or search endpoint exists yet.**
