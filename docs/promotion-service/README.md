...
See [planning/PROGRESS.md](planning/PROGRESS.md). **Phase 0, 1, and 2 (Domain Model) complete, 3/7. Phase 3 (Persistence) is in progress — 3.1 through 3.4 are done: all 103 entities have an `IEntityTypeConfiguration<T>`, `PromotionDbContext` is fully wired, the repository/Read/Write Persistence Service skeleton exists for all 11 true Aggregate Roots (empty — no query/command methods yet), and public Coupon search infrastructure (`CouponSearchDocument`/indexer/repository, Elasticsearch) is wired per [search/search-strategy.md](search/search-strategy.md). The full solution builds clean through `Promotion.API`. No CQRS, migration, or search endpoint exists yet.**
