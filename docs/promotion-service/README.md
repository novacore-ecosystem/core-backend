...
See [planning/PROGRESS.md](planning/PROGRESS.md). **Phase 0-3 complete (3/7 — see planning/PROGRESS.md for why the denominator only counts numbered phases 1-7). Persistence (Phase 3) is fully closed: all 103 entities have an `IEntityTypeConfiguration<T>`, `PromotionDbContext` is fully wired, the repository/Read/Write Persistence Service skeleton exists for all 11 true Aggregate Roots (empty — no query/command methods yet), and public Coupon search infrastructure (`CouponSearchDocument`/indexer/repository, Elasticsearch) is wired per [search/search-strategy.md](search/search-strategy.md) and summarized in [persistence/persistence-strategy.md](persistence/persistence-strategy.md)'s close-out section. The full solution builds clean through `Promotion.API`. Next: Phase 4 — Application/CQRS/API Skeleton. No CQRS, migration, or search endpoint exists yet.**
