...
See [planning/PROGRESS.md](planning/PROGRESS.md). **Phase 0-3 complete, Phase 4 (Application/CQRS/API) in progress — see planning/PROGRESS.md for how the "X / 7" marker works (current-phase position, not a completed count).** Persistence (Phase 3) is fully closed. Phase 4.1 built the representative CQRS/Minimal API skeleton; Phase 4.2 implemented the first real feature — Coupon Management (Create/Get/Update/Disable/List/Translate); Phase 4.3 connected Phase 3.4's Elasticsearch infrastructure into a real public Coupon search endpoint (`GET /coupons/search`); **Phase 4.4 added the Coupon validate/redeem lifecycle (`GET /coupons/validate`, `POST /coupons/redeem`), transactional/idempotent/concurrency-safe, reusing the Domain's existing `RecordUsage`/`CouponUsage`** — per [cqrs/cqrs-strategy.md](cqrs/cqrs-strategy.md). The full solution builds clean through `Promotion.API`. No Promotion-engine business logic (eligibility, discount calculation, stacking) or migration exists yet.**
