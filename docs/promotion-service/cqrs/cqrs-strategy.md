...
## Phase 5.1 — Promotion Rule & Discount Evaluation Core (built)

Real CQRS features for the `Promotion` aggregate — Management (`CreatePromotion`/`GetPromotion`/`ListPromotions`/`UpdatePromotion`/`EnablePromotion`/`DisablePromotion`/`CancelPromotion`), Approval (`SubmitPromotion`/`ApprovePromotion`/`RejectPromotion`), and evaluation (`EvaluatePromotionsQuery` → `POST /promotions/evaluate`), all following the same flow every prior phase established. Business-capability detail (lifecycle/approval model, targeting, conditions, discount, priority/stacking, evaluation algorithm) is documented separately in [../promotion-engine/promotion-engine-core.md](../promotion-engine/promotion-engine-core.md), not repeated here, per this phase's own instruction not to re-explain architecture already documented elsewhere.

One CQRS-shape note worth recording here: Submit/Approve/Reject are the first Handlers in Promotion Service to orchestrate **two independent aggregate roots** (`Promotion` and the general-purpose `ApprovalWorkflow`) in a single `IUnitOfWork.ExecuteTransactionAsync` call. Both `IPromotionWriteService` and `IApprovalWorkflowWriteService`'s Submit/Approve/Reject-related methods stage only (no bare `SaveChangesAsync`) specifically because of this — the Handler, not either Persistence Service, owns the one transaction spanning both aggregates. The two aggregates still only ever reference each other by id (`Promotion.ApprovalWorkflowId`); no Domain method call crosses the aggregate boundary directly.

## What Phase 4.1-4.4 / 5.1 do not do

No business rule beyond what's documented in [../promotion-engine/promotion-engine-core.md](../promotion-engine/promotion-engine-core.md) is inferred into any handler. Coupon Management Foundation, Public Coupon Search, Coupon Lifecycle/Redemption, and the Promotion Rule & Discount Evaluation Core are deliberately just the administrative/lifecycle/evaluation operations the Domain already exposes — Coupon-/Campaign-triggered evaluation, full stacking optimization, and Rule/Condition interpretation are explicitly out of scope until a future prompt requests them, same boundary Payment's `CreatePayment`/`CreatePaymentIntent`/`CreateRefund` precedent set for its own feature rollout.
